def bottom_up_algorithm(action_table, goto_table, input):
    ...
    while run == True:
        ...
        if not action[1] in action_table:
            error_message = f"Erro léxico: o token '{action[1]}' não pertence à gramática."
            suggestion = "Verifique se o token está escrito corretamente ou se precisa ser adicionado à gramática."
        ...
        if action_movement[0][:8] == "REDUZIR":
            ...
            expected_tokens = [token for token, actions in action_table.items() 
                              if actions[action[0]] != "ERRO!" and token != "$"]
            
            error_message = f"Erro sintático: token inesperado '{action[1]}' na posição {pointer}."
            suggestion = f"Esperava encontrar: {', '.join(expected_tokens)}" if expected_tokens else "Revise a estrutura da sua entrada."
            
            step_by_step.append(f"A entrada não está correta.")
            step_by_step_detailed.append([error_message, suggestion])
            detailed_steps.append(
                {
                    "stepByStep": step_by_step.copy(),
                    "stepByStepDetailed": step_by_step_detailed.copy(),
